...
`@rmem/encoding`. Фактичне versioning, tag policy, publish gate і recovery workflow належать до
...
- encoding label helpers `normalizeEncodingLabel`, `tryNormalizeEncodingLabel`,
  `aliasesForEncoding`, `isRmemEncodingName`;
...
Built-in profiles v1: `rmem`, `strictUtf8`, `legacyCyrillic`, `webCompat`.
...
- Non-exact backends не можуть задовольнити `sourceMap: "exact"` для `rmem` parser integration.
